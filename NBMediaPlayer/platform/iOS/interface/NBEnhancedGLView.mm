...
@interface NBGLThread : NSThread {
    ...
    __weak NBEnhancedGLView* mWeakGLView;
}

- (instancetype)initWithNBGLView:(NBEnhancedGLView*)glView;
...
- (nb_status_t)prepareRendererCtx {
    playerContext = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2 sharegroup:mainContext.sharegroup];
...
//    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    _renderInfo.x = 0;
    _renderInfo.y = 0;
    _renderInfo.width = framebufferWidth;
...
- (void*)getRendererCtx {
    return (__bridge void*)playerContext;
}
#pragma nbavplayer callback end
